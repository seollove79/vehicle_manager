...
// POST 데이터 처리
$modelsNum = isset($_POST["models_num"]) ? $_POST["models_num"] : exit("<script>alert('모델을 선택해주세요.');history.back();</script>");
...
// 기체 일련번호 중복 검사
$sqlStr = "SELECT * FROM vehicles WHERE vehicle_serial_num = :vehicle_serial_num";
...
$stmt->bindValue(':models_num', $modelsNum, PDO::PARAM_INT);
...
$stmt->execute();
$vehicles_num = $conn->lastInsertId();

// 파일 업로드 및 데이터베이스 저장 함수
function handleFileUpload($files, $conn, $vehicles_num, $category) {
    ...
    for ($i = 0; $i < $total; $i++) {
        ...
        if ($fileError === 0) {
            ...
            if (move_uploaded_file($fileTmpName, $fileDestination)) {
                ...
                $stmt = $conn->prepare($sqlStr);
                $stmt->execute([$vehicles_num, $fileDestination]);
            } else {
...
if (isset($_FILES['certification_initial_files'])) {
    handleFileUpload($_FILES['certification_initial_files'], $conn, $vehicles_num, 'certification_initial');
}

if (isset($_FILES['certification_1_files'])) {
    handleFileUpload($_FILES['certification_1_files'], $conn, $vehicles_num, 'certification_1');
}

if (isset($_FILES['certification_2_files'])) {
    handleFileUpload($_FILES['certification_2_files'], $conn, $vehicles_num, 'certification_2');
}

if (isset($_FILES['certification_3_files'])) {
    handleFileUpload($_FILES['certification_3_files'], $conn, $vehicles_num, 'certification_3');
}
...
$conn = null;
echo "<script>alert('기체가 등록되었습니다.');location.href='./list.php';</script>";
exit;
?>
